...
public class BlogPostDAO
{
	...
	{
		List<Document> comments = (ArrayList) postsCollection.find(eq("permalink", permalink)).first().get("comments");

		if (!comments.get(ordinal).containsKey("num_likes"))
			comments.get(ordinal).append("num_likes", 1);
		else
			comments.get(ordinal).replace("num_likes", Integer.parseInt(comments.get(ordinal).get("num_likes")
					                                                            .toString()) + 1);

		postsCollection.updateOne(eq("permalink", permalink), new Document("$unset", new Document("comment", 1)));
		postsCollection.updateOne(eq("permalink", permalink), new Document("$set", new Document("comments",
		                                                                                        comments)));
	}
}
